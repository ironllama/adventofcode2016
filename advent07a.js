...
let arrAlpha = ['aa', 'bb', 'cc', 'dd', 'ee', 'ff', 'gg', 'hh', 'ii', 'jj', 'kk', 'll', 'mm', 'nn', 'oo', 'pp', 'qq', 'rr', 'ss', 'tt', 'uu', 'vv', 'ww', 'xx', 'yy', 'zz'];
...
for (let i = 0; i < arrVal.length; i++) {
    ...
    for (let j = 0; j < arrAlpha.length; j++) {
        lastFound = 0;
        while (arrVal[i].indexOf(arrAlpha[j], lastFound) >= 0) {
            const foundHere = arrVal[i].indexOf(arrAlpha[j], lastFound);
            const testStr = arrVal[i].substr(foundHere-1, 4);
            //console.log("TESTING: " + testStr);
            const isAbba = checkAbba(testStr);
            //if (isAbba) console.log("ABBA: " + testStr);
            if (isAbba) {
                goodAbba = testStr;
                for (let k = 0; k < startBrackets.length; k++) { if (foundHere > startBrackets[k] && foundHere < endBrackets[k]) { continue eachVal; }}
            }
            lastFound = foundHere + 1;
        }
    }
    if (goodAbba !== "")
    {
        foundTLS++;
        console.log ("GOOD ABBA TLS: [" + i + "][" + goodAbba + "]");
    }
}

console.log("TOTAL TLS: " + foundTLS);

function checkAbba(string) {
    const allChars = string.split("");
    if (allChars[0] != "[" && allChars[0] !== allChars[1] && (allChars[0] + allChars[1]) === (allChars[3] + allChars[2])) return true;
    else return false;
}