...
for (let i = 0; i < arrVal.length; i++) {
    const thisVal = arrVal[i].split("[");
    const checkAlpha = thisVal[1].substr(0, 5);
    
    const noDash = thisVal[0].replace(/-/g, "");
    //process.stdout.write ("noDash [" + noDash + "]");
    
    let onlyAlpha = noDash.replace(/\d+$/, "");
    const arrAlpha = onlyAlpha.split("").sort();
    const onlyNum = noDash.replace(/^\D+/, "");
    
    let newAlpha = [];
    let currAlpha = "";
    let currCount = 0;
    for (let j = 0; j < arrAlpha.length; j++) {
        let thisAlpha = arrAlpha[j];
        if (currAlpha === "") {
            currAlpha = thisAlpha;
            currCount++;
        }
        else {
            if (currAlpha === thisAlpha) currCount++;
            else {
                newAlpha.push(currCount + currAlpha);
                currAlpha = thisAlpha;
                currCount = 1;
            }
        }
    ...
    newAlpha = newAlpha.sort(function(a, b) {
    ...
    //console.log (" onlyNum [" + thisVal[1].substr(0, 5) + "]");
    //console.log (" arrAlpha [" + arrAlpha.join("") + "]");
    //console.log (" onlyNum [" + onlyNum + "]");
    
    //console.log (" newAlpha [" + newAlpha.join("") + "]");
    let newCheck = "";
    for (let k = 0; k < 5; k++) { newCheck += newAlpha[k].charAt(1); }
    
    //console.log("CHECKSUMS: [" + newCheck + "][" + checkAlpha + "]");
    if (newCheck === checkAlpha) {
        total += parseInt(onlyNum);

        //let ex = 'qzmt-zixmtkozy-ivhz-343';
        //let alphaDashes = ex.replace(/-\d+$/, "");
        //let exNum = parseInt(ex.replace(/^\D+/, ""));
        //let cycNum = exNum % 26;
        let alphaDashes = thisVal[0].replace(/-\d+$/, "");
        let cycNum = onlyNum % 26;
        
        let letters = alphaDashes.split("");
        let newStr = "";
        for (let m = 0; m < letters.length; m++) {
            if ( letters[m] !== '-')
            {
                let thisCode = letters[m].charCodeAt(0) + cycNum;
                if (thisCode > 122) thisCode = (96 + (thisCode % 122));
                newStr += String.fromCharCode(thisCode);
            }
            else newStr += letters[m];
        }
        if (newStr.indexOf("north") > -1) north += newStr + "[" + onlyNum + "]\n";
        console.log("NEW: [" + newStr + "][" + onlyNum + "]");
    }
}
