...
for (let i = 0; i < arrVal.length; i++) {
    ...
    let lastFound = 0;
    while (arrVal[i].indexOf("[", lastFound) >= 0) {
        lastFound = arrVal[i].indexOf("[", lastFound);
        startBrackets.push(lastFound);
        lastFound++;
    }
    //console.log("START HYPER: " + JSON.stringify(startBrackets));
    lastFound = 0;
    while (arrVal[i].indexOf("]", lastFound) >= 0) {
        lastFound = arrVal[i].indexOf("]", lastFound);
        endBrackets.push(lastFound);
        lastFound++;
    }
    //console.log("END HYPER: " + JSON.stringify(endBrackets));
    
    let strOutBrackets = "";
    let strInBrackets = "";
    let currIdxBracket = 0;
    for (let j = 0; j < arrVal[i].length; j++) {
        if (j < startBrackets[currIdxBracket] || currIdxBracket >= startBrackets.length) strOutBrackets += arrVal[i][j];
        else if (j > startBrackets[currIdxBracket] && j < endBrackets[currIdxBracket]) strInBrackets += arrVal[i][j];
        else if (j === endBrackets[currIdxBracket]) {
            strOutBrackets += "-";
            strInBrackets += "-";
            currIdxBracket++;
        }
    }
    //console.log("Outside: [" + strOutBrackets + "] Inside:[" + strInBrackets + "]");
    
    const allOutAba = getAbas(strOutBrackets);
    const allInAba = getAbas(strInBrackets);
    //console.log("OutABA: [" + allOutAba.join('') + "] InABA:[" + allInAba.join('') + "]");
    
    for (let k = 0; k < allOutAba.length; k++) {
        const thisAba = allOutAba[k].split("");
        const idxBab = allInAba.indexOf(thisAba[1] + thisAba[0] + thisAba[1]);
        if ( idxBab >= 0) {
            foundSSL++;
            console.log("GOT ONE! ABA[" + thisAba.join('') + "][" + allInAba[idxBab] +"]");
            continue eachVal;
        }
    }
}

console.log("TOTAL SSL: " + foundSSL);

function getAbas(thisString) {
    const resArr = [];
    for (let i = 0; i < thisString.length; i++) {
        if (thisString[i] === thisString[i+2] && thisString[i] !== thisString[i+1]) resArr.push(thisString.substr(i, 3));
    }
    return resArr;
}